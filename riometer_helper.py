...
def logging(infft,renormal,prefix,freq,bw,prate,longitude):
    ...
    if ((time.time() - last_time) >= 1.0):
        ...
        if (seconds != 0 and (seconds % 1 == 0)):
            ...
            #
            # Derive 1sec reference value from fast_pHz values
            #
            r = 0.0
            ...
            #
            sortedp = [x[0] for x in fast_data_pHz]
            ...
            #
            # Pull the mid 3 values and average them
            #
            p = sortedp[len(sortedp)/2]
            p += sortedp[len(sortedp)/2+1]
            p += sortedp[len(sortedp)/2-1]
            p /= 3.0
